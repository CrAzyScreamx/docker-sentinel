...
| **2 - Static Analysis** | 9 direct Python tools | Runs all nine static analyzers (see below) concurrently with zero LLM overhead. |
| **3 - URL Validator** | Deterministic pipeline *(conditional)* | Three-stage pipeline: (1) filters RFC 1918, loopback, APIPA, and known public DNS servers; (2) resolves domain names to IPv4 via Google DNS over HTTPS; (3) checks each public IP against Spamhaus ZEN using GNS authoritative nameservers directly. Skipped if no URLs were extracted. |
